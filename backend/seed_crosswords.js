...
function generateCrosswordItems(wordsWithClues) {
    const items = [];
        ...
        for (const item of items) {
            ...
            for (let i = 0; i < item.word.length; i++) {
                ...
                if (matchIdx !== -1) {
                    ...
                    // Check bounds
                    if (startRow < 0 || startCol < 0 || startRow + (newDir === "VERTICAL" ? w.length : 1) > 20 || startCol + (newDir === "HORIZONTAL" ? w.length : 1) > 20)
                        continue;
                    // Check collision
                    let canPlace = true;
