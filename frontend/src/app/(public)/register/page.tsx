...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("https://biology-backend-vw8k.onrender.com/api/auth/register", {
        method: "POST",
      ...
      if (!res.ok) {
        throw new Error(data.error || "Nimadir xato ketdi");
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-foreground/80">Ism</label>
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-foreground/80">Familiya</label>
                <div className="relative">
                  <User className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-foreground/40" />
                  <input
                    type="text"
