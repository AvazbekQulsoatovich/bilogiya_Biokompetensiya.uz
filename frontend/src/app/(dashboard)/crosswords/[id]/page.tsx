...
export default function CrosswordSolverPage() {
  ...
  const fetchCrossword = async () => {
    try {
      const res = await fetch(`https://biology-backend-vw8k.onrender.com/api/crosswords/${params.id}`);
  ...
  const submitCompletion = async () => {
    try {
      ...
      const res = await fetch(`https://biology-backend-vw8k.onrender.com/api/crosswords/${params.id}/submit`, {
        method: "POST",
        headers: {
